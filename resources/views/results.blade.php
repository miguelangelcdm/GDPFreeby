      ...
      <div class="flex items-center gap-4">
        ...
        <div>
          <form method="POST" action="{{ url('/') }}">
            @csrf
            <input type="hidden" name="search_value" value="{{ isset($searchValue) ? $searchValue / -100 : null }}">
            <input type="hidden" name="threshold" value="{{ isset($threshold) ? $threshold / 100 : null }}">
            <input type="hidden" name="currency" value="{{$currency}}" />
            <button type="submit"
              class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Te sirvio?</button>
          </form>
        </div>
      </div>
    </div>
    <div class="mx-auto p-6 lg:p-8 flex-col flex pt-24 lg:mt-12" style="width: -webkit-fill-available">
            ...
            <form action="{{ url('/process-csv') }}" method="post" enctype="multipart/form-data">
              ...
              <div class="mb-6">
                <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Especifique
                  el currency</label>
                <div
                  class="grid grid-cols-2 gap-2 rounded-xl bg-gray-200 p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white">
                  <div>
                    <input type="radio" name="currency" id="pesos" value="1" class="peer hidden" {{ $currency == 1 ? 'checked' : '' }} />
                    <label for="pesos"
                  ...
                  <div>
                    <input type="radio" name="currency" id="soles" value="2" class="peer hidden" {{ $currency == 2 ? 'checked' : '' }} />
                    <label for="soles"
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              ...
              <thead
                ...
                <tr>
                  ...
                  <th scope="col" class="px-6 py-3">Apuesta</th>
              ...
              <tbody>
                @foreach ($matrix as $row)
                  <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    ...
                    {{-- <td class="px-6 py-4">{{ $row['GInstance'] }}</td> --}}
                    ...
                    <td class="px-6 py-4">{{ $row['ObjectId'] }}</td>
                  </tr>
                @endforeach
              </tbody>
            </table>
      ...
      <div class="flex lg:justify-around mt-10 px-0 justify-center">
